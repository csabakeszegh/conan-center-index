...
class SDLMixerConan(ConanFile):
    ...
    topics = ("sdl3", "sdl", "mixer", "audio", "multimedia", "sound", "music")
    ...
    def requirements(self):
        self.requires("sdl/[>=3.2 <4]", transitive_headers=True, transitive_libs=True)
    ...
    def generate(self):
        ...
        tc.variables["SDLMIXER_DEBUG_POSTFIX"] = ""
        tc.variables["SDLMIXER_VENDORED"] = False
        tc.variables["SDLMIXER_SAMPLES"] = False
        tc.variables["SDLMIXER_CMD"] = self.options.cmd
        # WAVE
        tc.variables["SDLMIXER_WAVE"] = self.options.wav
        # FLAC
        tc.variables["SDLMIXER_FLAC"] = self.options.flac
        # GME
        tc.variables["SDLMIXER_GME"] = self.options.gme
        # MOD
        tc.variables["SDLMIXER_MOD"] = self.options.modplug and bool(self.options.xmp)
        tc.variables["SDLMIXER_MOD_MODPLUG"] = self.options.modplug
        tc.variables["SDLMIXER_MOD_XMP"] = self.options.xmp == "libxmp"
        tc.variables["SDLMIXER_MOD_XMP_LITE"] = self.options.xmp == "libxmp-lite"
        # MP3
        tc.variables["SDLMIXER_MP3"] = self.options.mpg123 or self.options.minimp3
        tc.variables["SDLMIXER_MP3_MPG123"] = self.options.mpg123
        tc.variables["SDLMIXER_MP3_MINIMP3"] = self.options.minimp3
        # MIDI
        tc.variables["SDLMIXER_MIDI"] = self.options.get_safe("nativemidi", False) or self.options.get_safe("tinymidi", False) or self.options.fluidsynth
        tc.variables["SDLMIXER_MIDI_FLUIDSYNTH"] = self.options.fluidsynth
        tc.variables["SDLMIXER_MIDI_TIMIDITY"] = self.options.get_safe("tinymidi", False)
        tc.variables["SDLMIXER_MIDI_NATIVE"] = self.options.get_safe("nativemidi", False)
        # OPUS
        tc.variables["SDLMIXER_OPUS"] = self.options.opus
        # VORBIS
        if self.options.vorbis == "stb":
            tc.variables["SDLMIXER_VORBIS"] = "STB"
        elif self.options.vorbis == "vorbisfile":
            tc.variables["SDLMIXER_VORBIS"] = "VORBISFILE"
        elif self.options.vorbis == "tremor":
            tc.variables["SDLMIXER_VORBIS"] = "TREMOR"
        else:
            tc.variables["SDLMIXER_VORBIS"] = False
        # WavPack
        tc.variables["SDLMIXER_WAVPACK"] = self.options.wavpack

        # TODO: add support for dynamic loading of dependencies
        tc.variables["SDLMIXER_DEPS_SHARED"] = False
        tc.variables["SDLMIXER_FLAC_LIBFLAC_SHARED"] = False
        tc.variables["SDLMIXER_GME_SHARED"] = False
        tc.variables["SDLMIXER_MIDI_FLUIDSYNTH_SHARED"] = False
        tc.variables["SDLMIXER_MIDI_TIMIDITY_SHARED"] = False
        tc.variables["SDLMIXER_MOD_MODPLUG_SHARED"] = False
        tc.variables["SDLMIXER_MOD_XMP_SHARED"] = False
        tc.variables["SDLMIXER_MP3_MPG123_SHARED"] = False
        tc.variables["SDLMIXER_OGG_SHARED"] = False
        tc.variables["SDLMIXER_OPUS_SHARED"] = False
        tc.variables["SDLMIXER_SNDFILE_SHARED"] = False
        tc.variables["SDLMIXER_VORBIS_TREMOR_SHARED"] = False
        tc.variables["SDLMIXER_VORBIS_VORBISFILE_SHARED"] = False
        tc.variables["SDLMIXER_WAVPACK_SHARED"] = False
        tc.generate()
    ...
    def package_info(self):
        ...
        if not self.options.shared:
            self.cpp_info.set_property("cmake_target_aliases", ["SDL3_mixer::SDL3_mixer-static"])

        if is_msvc(self) and not self.options.shared:
            self.cpp_info.libs = ["SDL3_mixer-static"]
        else:
            self.cpp_info.libs = ["SDL3_mixer"]
        self.cpp_info.includedirs.append(os.path.join("include", "SDL3_mixer"))
        ...
        self.cpp_info.names["cmake_find_package"] = "SDL3_mixer"
        self.cpp_info.names["cmake_find_package_multi"] = "SDL3_mixer"
